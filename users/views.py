...
def github_callback(request):
    ...
    try:
        ...
        if code is not None:
            token_request = requests.post(f"http://github.com/login/oauth/access_token?client_id={client_id}&client_secret={client_secret}&code={code}",
            ...
            if error is not None:
                ...
                profile_request = requests.get("http://api.github.com/user", 
                ...
                if username is not None:
                    ...
                    try:
                        ...
                        user.save()
                    login(request, user)
                    messages.success(request, f"안녕하세요 {user.first_name}")
                    return redirect(reverse("core:home"))                    
                else:
                   raise GithubException()
        else:
...
def kakao_callback(request):
    try:
        ...
        profile_image = properties.get("profile").get("thumbnail_image_url")
...
class UpdatePasswordView(mixin.EmailLoginOnlyView, mixin.LoggedInOnlyView, SuccessMessageMixin, PasswordChangeView):

    template_name = "users/update-password.html"
    sucess_message = "Password updated"

    def get_form(self, form_class=None):
        form = super().get_form(form_class=form_class)
        form.fields["old_password"].widget.attrs = {"placeholder": "현재 비밀번호"}
        form.fields["new_password1"].widget.attrs = {"placeholder": "새 비밀번호"}
        form.fields["new_password2"].widget.attrs = {"placeholder": "새 비밀번호 다시 입력"}
        return form
